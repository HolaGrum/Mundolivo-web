            ...
            @if($item->hasChildren())
                @if($item->childrenAllowed()->count())
                    <span class="nav-item nav-dropdown{{ $item->hasActiveChild() ? ' open' : '' }}">
                        <a href="#sidebar-submenu-{{$item->name}}" class="nav-link nav-dropdown-toggle" aria-expanded="{{ $item->hasActiveChild() ? 'true' : 'false' }}">
                            @if($item->data('icon'))
                                {!! icon($item->data('icon')) !!}
                            @endif
                            {!! $item->title !!}
                        </a>
                        <div class="nav-dropdown-items{{ $item->hasActiveChild() ? ' show' : '' }}" id="sidebar-submenu-{{$item->name}}">
